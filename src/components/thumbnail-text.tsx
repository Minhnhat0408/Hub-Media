...
function ThumbNailText({
    ...
    return (
        <article className={cn('md:group relative flex h-[500px] md:py-0 py-6 xl:flex-1 md:w-1/2 w-full   flex-col border-r-[1px] border-muted-foreground',className)}>
            <div className="flex h-fit w-full flex-col justify-center xl:items-start lg:items-center px-[40px]  pb-5 ">
            ...
            <Link href={pathRemainLang({ pathname, newpath: href })} className="relative flex flex-1 cursor-pointer items-center justify-center ">
                ...
                <Image
                    ...
                    height="0"
                    sizes="100vw"
                    className=" absolute  md:h-2/3 md:w-2/3 w-full h-full  object-cover md:opacity-0  duration-500  group-hover:h-full group-hover:w-full group-hover:opacity-100 "
                />
            </Link>
        </article>
    );
