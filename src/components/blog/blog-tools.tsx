...
export default function BlogTools({
    ...
    useEffect(() => {
        const res = localStorage.getItem('recentpost')
    ...
    useEffect(() => {
        if (searchValue.trim()) {
            ...
            const fetchData = async (value: string) => {
                let res = allPosts.filter((item) => item.title.toLowerCase().includes(value.toLowerCase()));
                setTimeout(() => {
                    setSearchResult(res);
                    setLoading(false);
                }, 500);
    ...
    return (
        <div className="h-full w-[350px] space-y-12 border-2 border-gradient px-10 py-16 xl:w-[400px] ">
            ...
            <Image
                src={
                    'https://firebasestorage.googleapis.com/v0/b/hub-media-207ea.appspot.com/o/images%2FIMG_8302.JPG?alt=media&token=f318bef8-861d-4499-b4b5-8e8a2064e530&_gl=1*6vzl6e*_ga*MjEzMTY3MzA4MS4xNjkxMzM2Nzk5*_ga_CW55HF8NVT*MTY5ODkzOTIwNS4zMTkuMS4xNjk4OTM5MzY2LjQ0LjAuMA..'
                }
