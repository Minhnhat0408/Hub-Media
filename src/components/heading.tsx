        ...
        <header ref={ref} className={cn("flex flex-col items-center justify-center pb-20",className)}>
            ...
            <div className={cn(' h-fit  relative flex text-6xl justify-center  font-bold ')}>
                {description.split(' ').map((word, index) => {
                    let time = index <1 ? 80: 80*description.split(' ').splice(0,index).reduce((a,b)=>{
                        return a + b.length
                    },0);
                    return (
                        <span key={index} className="mr-4 flex">
                            {word.split('').map((char, ind) => {
                                ...
                                return (
                                    <span
                                        className={cn("opacity-0 py-2 translate-x-[50px] scale-150 duration-700  " ,isInView && ' flex opacity-1 translate-x-0  super scale-100')}
